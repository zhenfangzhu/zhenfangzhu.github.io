...
所有已发布笔记同时生成独立页面、1200×630 PNG 封面与站点地图条目。目录只展示日期、标题和摘要；完整正文在文章页阅读。每篇文章有自己的标题、描述、canonical 地址和 Article / BreadcrumbList 结构化数据；目录生成 CollectionPage / ItemList。现有日期是笔记日期，不推断为发布时间。提交文本给 Codex 时只需提供标题、日期、来源和正文，Codex 可整理为此格式。

## 交互约定

- 点击目录标题进入独立文章；原生链接支持键盘、复制网址和在新标签页打开。
- JavaScript 不可用时，完整目录和文章仍可阅读、导航。
- 搜索匹配目录中的标题、日期、摘要，以及已发布笔记各节标题、摘录和批注。构建时将正文索引转义后写入卡片的 `data-search-text` 属性，无需额外请求文章；索引不进入目录可见文字或 ARIA 标签。待整理笔记不索引正文。
- 搜索默认只显示放大镜，点击后展开并聚焦；按 Esc 或再次点击放大镜会清空筛选、收起输入框。
- 从搜索结果进入文章，再用顶部返回链接或浏览器后退，会恢复搜索条件、展开状态和滚动位置；刷新目录也保留当前状态。浏览历史与当前标签页的 sessionStorage 保存这些状态，存储不可用时仍可正常搜索和导航。
- 直接打开目录或文章底部的「全部回声」显示完整列表；直接访问带锚点的网址仍定位对应条目，不套用旧搜索。待整理条目没有文章链接。
