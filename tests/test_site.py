...
class SiteContractTests(unittest.TestCase):
    ...
    def test_founder_dna_is_deployable_and_keeps_data_local(self):
        ...
        self.assertIn("const STORAGE_KEY='founder-dna-v2'", app)
    ...
    def test_founder_dna_is_a_public_assessment(self):
        app = read("founder-dna/index.html")
        translations = read("static/js/founder-i18n.js")
        self.assertIn("项目名称或类型", app)
        self.assertIn('id="welcome-founder-a"', app)
        self.assertIn('id="welcome-founder-b"', app)
        self.assertIn('id="welcome-founder-c"', app)
        self.assertIn("创建团队评估", app)
        self.assertIn("你的回答只保存在当前浏览器，不会上传到服务器。", app)
        self.assertIn("DEFAULT_NAMES=['创始人 A','创始人 B','创始人 C']", app)
        self.assertIn('"创建团队评估": "Create team assessment"', translations)
        self.assertNotIn("线下 AI 编程教育", app)
        self.assertNotIn("约 200 名付费用户", app)
        self.assertNotIn("当家长说", app)
        self.assertNotIn("孩子的课堂反应", app)
        self.assertNotIn("完成一次抖音增长实验", app)

    def test_founder_dna_supports_two_to_six_founders(self):
        app = read("founder-dna/index.html")
        translations = read("static/js/founder-i18n.js")
        self.assertIn("const FOUNDER_IDS=['A','B','C','D','E','F']", app)
        self.assertIn("const MIN_FOUNDERS=2,MAX_FOUNDERS=6", app)
        self.assertIn("data-welcome-add", app)
        ...
        self.assertNotIn("evaluations.length===9", app)
